...
export default class Question extends Component {
  ...
  handleSubmit() {
    ...
    showLoading();
    reqwest({
      url: `${hostname}/api/question/reply`,
      method: 'get',
      type: 'jsonp',
    ...
    return (
      <div class="issue-detail">
        <div className="display-wrap">
          <div className="title"><span className="label">问题标题：</span><span>{data.title}</span></div>
          <div className="author-name"><span className="label">提问人姓名：</span><span>{data.publisher.name}</span></div>
          <div className="author-phone"><span className="label">提问人电话：</span><span>{data.publisher.phone}</span></div>
          <div className="author-address"><span className="label">提问人住址：</span><span>{data.publisher.address}</span></div>
          <div className="date"><span className="label">发布日期：</span><span>{data.date}</span></div>
          <div className="content"><span className="label">问题详情：</span><span>{data.content}</span></div>
          {imgEls.length > 0 ? 
